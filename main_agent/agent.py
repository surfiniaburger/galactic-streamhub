...
def create_streaming_agent_with_mcp_tools(
    ...
    # Determine the model to use. Use the override if provided, otherwise default to the streaming model.
    final_model_id = model_override if model_override else MODEL_ID_STREAMING

    # 5. Create the Root Agent (mcp_streaming_assistant)
    # This is your main, user-facing multimodal agent.
    root_agent = LlmAgent(
        model=final_model_id, # Use the determined model ID
...
# --- Standalone Agent Instance for ADK Tools ---
# This section creates a `root_agent` instance that `adk web` and `adk eval`
# can discover and use. It uses a default set of MCP toolsets that are
# started by the ADK runner. Your main.py application will still create its
# own agent instance with dynamically configured toolsets during its lifespan.
import os
from google.adk.tools.mcp_tool.mcp_toolset import StdioConnectionParams

# Define default MCP server configurations for standalone testing.
# The paths are relative to the project root where `adk` is run.
...
# Instantiate default toolsets that the ADK runner will manage.
default_mcp_toolsets = [
    MCPToolset(connection_params=weather_server_params),
    MCPToolset(connection_params=ct_server_params)
]

# Add Maps toolset if the API key is available in the environment.
...
if google_maps_api_key_from_env:
    ...
    default_mcp_toolsets.append(MCPToolset(connection_params=maps_server_params))

# Create the root_agent instance that ADK tools will discover and use.
root_agent = create_streaming_agent_with_mcp_tools(
    loaded_mcp_toolsets=default_mcp_toolsets,
    model_override=GEMINI_PRO_MODEL_ID # Use the non-live model for ADK tools
)
