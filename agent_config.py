...
7.  **Handling Video Summary Request**:
    *   The `PubMedRAGAgent` might ask the user "Would you like a video summary of these findings?".
    *   If the user's *next* message is an affirmative response (e.g., "yes, please", "sure", "generate video"), you MUST call the `VideoReportAgent` tool.
    *   Before calling `VideoReportAgent`, ensure the necessary inputs are in session state:
        *   `ctx.session.state['video_report_text']` (this should be the full text summary previously generated by `PubMedRAGAgent` - you might need to store this from `PubMedRAGAgent`'s output).
        *   `ctx.session.state['video_chart_image_urls']` (a list of chart URLs previously generated - also needs to be stored from `PubMedRAGAgent`'s output).
    *   The `VideoReportAgent` will then produce the video (or a status message for now). Present its output to the user.
...
5.  **Combine Everything:** Your FINAL response to the user MUST be a single, coherent message that combines:
    ...
    d. After presenting the above, ask the user: "Would you like a video summary of these findings?"
